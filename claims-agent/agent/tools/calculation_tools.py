...
def calculate_compensation(algorithm_id: str, bill_data: dict, policy_data: dict) -> dict:
    ...
    # 参数提取
    drug_price = safe_decimal(bill_data.get("drug_unit_price", 0))
    quantity = safe_decimal(bill_data.get("quantity", 0))
    payout_ratio = safe_decimal(policy_data.get("payout_ratio", 0.9))
    remaining_coverage = safe_decimal(policy_data.get("remaining_coverage", 50000))
    deductible_balance = safe_decimal(policy_data.get("deductible_balance", 0))
    social_security = safe_decimal(bill_data.get("social_security", 0))
    self_pay = safe_decimal(bill_data.get("self_pay", 0))
    third_party = safe_decimal(bill_data.get("third_party_pay", 0))
    total_amount = safe_decimal(bill_data.get("total_amount", 0))
    category_b = safe_decimal(bill_data.get("category_b_self_pay", 0))
...
def aggregate_liabilities(case_id: str, policy_results: list[dict]) -> dict:
    ...
    # 按主次险顺序排序
    ordered = sorted(policy_results, key=lambda p: p.get("insurance_order", 99) or 99)
